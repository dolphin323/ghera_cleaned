...
public class Tests {
    ...
    public void testPresenceOfVulnerability() throws UiObjectNotFoundException {
        ...
        UiObject button = mDevice.findObject(new UiSelector().resourceId(BENIGN_APP_PACKAGE + ":id/benign"));
        ...
        onWebView()
                ...
                .check(webMatches(getText(), containsString("username=John Doe")));
        mDevice.pressBack();
        InstrumentationRegistry.getInstrumentation().waitForIdleSync();

        button = mDevice.findObject(new UiSelector().resourceId(BENIGN_APP_PACKAGE + ":id/malicious"));
        assertNotNull(button);
        button.clickAndWaitForNewWindow();
        onWebView()
                .withElement(findElement(Locator.ID, "putButton"))
        ...
        button = mDevice.findObject(new UiSelector().resourceId(BENIGN_APP_PACKAGE + ":id/benign"));
        assertNotNull(button);
        button.clickAndWaitForNewWindow();
        ...
        onWebView()
                ...
                .perform(webClick())
                .withElement(findElement(Locator.ID, "id"))
                .check(webMatches(getText(), containsString("username=Jhontu")));
    }
}
