...
public class Tests {
    ...
    public void testAbsenceOfVulnerability() throws UiObjectNotFoundException {
        ...
        UiObject send_pi_btn = mDevice.findObject(new UiSelector().resourceId(BENIGN_APP_PACKAGE + ":id/button"));
        assertThat(send_pi_btn, notNullValue());
        send_pi_btn.click();
        InstrumentationRegistry.getInstrumentation().waitForIdleSync();
        UiObject msg_view = mDevice.findObject(new UiSelector().resourceId(BENIGN_APP_PACKAGE + ":id/disp"));
        assertThat(msg_view, notNullValue());
        assertEquals("SomeActivity", msg_view.getText());
    }
}
