...
public class Tests {
    ...
    public void testPresenceOfVulnerability() throws UiObjectNotFoundException {
        ...
        UiObject btn = mDevice.findObject(new UiSelector().resourceId(MALICIOUS_APP_PACKAGE + ":id/button"));
        assertThat(btn, notNullValue());
        btn.click();
        mDevice.pressBack();
        UiObject text = mDevice.findObject(new UiSelector().resourceId(MALICIOUS_APP_PACKAGE + ":id/disp"));
        assertThat(text, notNullValue());
        assertEquals("sensitive information", text.getText());
    }
}
