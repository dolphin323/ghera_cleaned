...
public class Tests {
    ...
    public void testPresenceOfVulnerability() throws UiObjectNotFoundException {
        ...
        UiObject btn = mDevice.findObject(new UiSelector()
                .resourceId("edu.ksu.cs.malicious:id/scenario1"));
        btn.click();
        InstrumentationRegistry.getInstrumentation().waitForIdleSync();
        startApp(BENIGN_APP_PACKAGE);
        UiObject getFile_btn = mDevice.findObject(new UiSelector()
                .resourceId("edu.ksu.cs.benign:id/getFile"));
        getFile_btn.click();
        UiObject text = mDevice.findObject(new UiSelector()
                .resourceId("edu.ksu.cs.benign:id/file_status"));
