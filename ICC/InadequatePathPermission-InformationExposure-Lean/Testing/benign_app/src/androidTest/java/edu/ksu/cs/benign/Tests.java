...
public class Tests {
    ...
    public void testPresenceOfVulnerability() throws UiObjectNotFoundException {
        ...
        UiObject getSSN_btn = mDevice.findObject(new UiSelector().resourceId(MALICIOUS_APP_PACKAGE + ":id/getSsn"));
        assertThat(getSSN_btn, notNullValue());
        getSSN_btn.click();
        UiObject dispSSN = mDevice.findObject(new UiSelector().resourceId(MALICIOUS_APP_PACKAGE + ":id/ssnDisp"));
        assertThat(dispSSN, notNullValue());
        assertEquals("123456789", dispSSN.getText());
