...
public class Tests {
    ...
    public void testPresenceOfVulnerability() throws UiObjectNotFoundException {
        ...
        UiObject injectFrag = mDevice.findObject(new UiSelector()
                .resourceId(MALICIOUS_APP_PACKAGE + ":id/inject_frag"));
        injectFrag.click();
        ...
        UiObject loadFrag = mDevice.findObject(new UiSelector()
                .resourceId("edu.ksu.cs.benign:id/load_frag"));
        loadFrag.click();

        UiObject msg = mDevice.findObject(new UiSelector()
                .resourceId("edu.ksu.cs.benign:id/email_msg"));
        assertEquals("Email sent successfully", msg.getText());
    }
}
