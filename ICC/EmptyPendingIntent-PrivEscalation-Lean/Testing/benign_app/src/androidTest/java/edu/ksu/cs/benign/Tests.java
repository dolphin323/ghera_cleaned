...
public class Tests {
    ...
    public void testPresenceOfVulnerability() throws UiObjectNotFoundException {
        ...
        UiObject send_pi_btn = mDevice.findObject(new UiSelector()
                .resourceId(BENIGN_APP_PACKAGE + ":id/send_pi_button"));
        assertThat(send_pi_btn, notNullValue());
        send_pi_btn.click();
        UiObject sens_activity_msg = mDevice.findObject(new UiSelector()
                .resourceId(BENIGN_APP_PACKAGE + ":id/sensitive_activity_msg"));
        assertTrue(sens_activity_msg.waitForExists(LAUNCH_TIMEOUT));
        assertEquals(msg, sens_activity_msg.getText());
    }
}
